...
export class Dimetric extends BaseAxonometric {
    ...
    constructor (center: Vector2, grid: Grid) {
        super(center, grid);
        if (grid.type !== 'DIMETRIC')
            throw new Error(`Cannot create a Dimetric cell for a "${grid.type}" grid`);
    }
    ...
    isAdjacent (other: Cell): boolean {
        ...
        return (
            Math.abs(xDiff - this.grid.dpi) < 0.1
            && Math.abs(yDiff - this.grid.dpi / 2) < 0.1
        );
    }
